             ...
             model = FALSE, x = FALSE, y = TRUE, parms, control, cost, ...)
{
    ...
    if (missing(method)) {
	    method <- (
            if (is.factor(Y) || is.character(Y)) "class"
    ...
    if (is.list(method)) {
        ...
        method <- "user"
            ...
            ## garbage collection
        init <- (
            if (missing(parms)) mlist$init(Y, offset, wt = wt)
    ...
    tfun <- function(x) {
	    if (is.matrix(x)) {
            return(rep(is.ordered(x), ncol(x)))
        } else {
            return(is.ordered(x))
        }
    }
    ...
    temp <- (
        if (nrow(rpfit$cptable) == 3L) c("CP", "nsplit", "rel error")
    ...
    functions <- (
        if (is.null(init$print)) list(summary = init$summary)
